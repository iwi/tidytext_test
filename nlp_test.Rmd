...
source("./weeknotes_analysis_tools.R")
```

```{r, message=FALSE, warning=FALSE}
weeknotes_username <- 'drryandunn'
```

```{r, message=FALSE, warning=FALSE}
weeknotes_ryan <- as_tibble(read_csv(paste('weeknotes_', weeknotes_username, '.csv', sep = ''), col_names = TRUE))

# Rename variables converting the original urls to season plus episode names
...
```{r}
length(weeknotes_ryan)
weeknotes_ryan %>% 
  gather("episode", "text", 1:length(weeknotes_ryan)) -> tidy_notes
```
...
tidy_notes %>% 
  ...
  mutate(text2 = gsub("‘", " ", text2)) -> tidier_notes
```
...
```{r}
data("stop_words")
other_stop_words <- c("i’d", "i’m", "i’ve", "’", "ve", "d", "ll", "m")
stop_words
```
...
tidy_tokens_tagged@TT.res %>%
  as_tibble() %>% 
  mutate(episode = tidy_tokens$episode, text = tidy_tokens$text) %>% 
  anti_join(stop_words, by = c("token" = "word")) %>% 
  filter(!(token %in% other_stop_words)) %>% 
